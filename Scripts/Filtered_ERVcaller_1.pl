...
while(<F1>){
  ...
  ### Checck the breakpoints detected
  if ($line[14] =~ "D") {
  ...
  if ($line[23] eq 0) {
    ...
    next;
  } elsif ($line[22] eq $depth && $line[23] eq 1 && $line[27] eq 0){
    next;
  } elsif ($line[22] eq $depth && $line[23] eq 1 && $line[27] > 0) {
    $false_or_not="F_depth";
  } elsif ( $line[24] + $line[25] >= $line[23]) {
  ...
  if ($read_len <=110 && $read_len >=75) {                     ### read length 75 bp to 110 bp;
