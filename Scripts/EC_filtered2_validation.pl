...
while(<CANDIDATE>){
 ...
 if($line[1] eq $read_name){
  ...
  elsif(exists($read{$name2}) && (($line[0] eq "F" && ${$read{$name2}}[0] eq "R") || ($line[0] eq "R" && ${$read{$name2}}[0] eq "F")) && $line[7] eq "="){
   if((($line[0] eq "F" && ${$read{$name2}}[0] eq "R") || ($line[0] eq "F" && ${$read{$name2}}[0] eq "R")) && $line[3] eq ${$read{$name2}}[6] && $line[4] eq ${$read{$name2}}[11] && $line[8] eq ${$read{$name2}}[7]){
     splice @{$read{$name2}},3,0,@line;
     ${$read{$name2}}[0]="P2";
     ${$read{$name2}}[2]+=$line[11];                                        }
   else{
