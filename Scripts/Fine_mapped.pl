...
while(<F1>) {
  @line=split;
  my @read_name = split /\|/, $line[1];           #### 02/17/2019
  if ($line[0] eq "O1"){
    @O1=@line;
    $order=1;
    ...
    ############## fr
      if ($line[3] eq "F" && $line[5] eq "R"){
       if ($line[2] eq "S1"){
         ...
         $rr[3]=$line[9]+$line[8];
         if ($rr[1] >0 || $rr[2]>0) {
           if ($line[7]+$line[6] <= $rr[1]) {
             $rr[1]=$line[7]+$line[6];
           }
           if ($line[7]+$line[6]+($line[10]-$line[6]+1) >= $rr[2]) {
             $rr[2]=$line[7]+$line[6]+($line[10]-$line[6]+1);
           }
         } else {
           $rr[1]=$line[7]+$line[6];$rr[2]=$line[7]+$line[6]+($line[10]-$line[6]+1);
         }
         if ($rr[4] >0 || $rr[5]>0){
           if ($line[9]+$line[8] <= $rr[4]) {
             $rr[4]=$line[9]+$line[8];
           }
           if ($line[9]+$line[8]+($line[12]-$line[8]+1) >= $rr[5]) {
             $rr[5]=$line[9]+$line[8]+($line[12]-$line[8]+1);
           }
         } else {
           $rr[4]=$line[9]+$line[8];
           $rr[5]=$line[9]+$line[8]+($line[12]-$line[8]+1);
        ...
        $fr[6]++;
        if ($fr[1] >0 || $fr[2]>0){
          if ($line[7]+$line[6] <= $fr[1]) {
            $fr[1]=$line[7]+$line[6];
          }
          if ($line[7]+$line[6]+($line[10]-$line[6]+1) >= $fr[2]) {
            $fr[2]=$line[7]+$line[6]+($line[10]-$line[6]+1);
          }
        } else {
          $fr[1]=$line[7]+$line[6];
          $fr[2]=$line[7]+$line[6]+($line[10]-$line[6]+1);
        ...
        if ($fr[4] >0 || $fr[5]>0) {
          if ($line[9]+$line[8] <= $fr[4]) {
            $fr[4]=$line[9]+$line[8];
          }
          if ($line[9]+$line[8]+($line[12]-$line[8]+1) >= $fr[5]) {
            $fr[5]=$line[9]+$line[8]+($line[12]-$line[8]+1);
          }
        } else {
          $fr[4]=$line[9]+$line[8];
          $fr[5]=$line[9]+$line[8]+($line[12]-$line[8]+1);
    ...
    ############## ff
      ...
      if($line[2] eq "S1"){
        ...
        if($rf[1] >0 || $rf[2]>0){
          if ($line[7]+$line[6] <= $rf[1]) {
            $rf[1]=$line[7]+$line[6];
          }
          if ($line[7]+$line[6]+($line[10]-$line[6]+1) >= $rf[2]) {
            $rf[2]=$line[7]+$line[6]+($line[10]-$line[6]+1);
          }
        } else {
          $rf[1]=$line[7]+$line[6];
          $rf[2]=$line[7]+$line[6]+($line[10]-$line[6]+1);
        ...
        if ($rf[4] >0 || $rf[5]>0) {
          if ($line[9]+$line[8] <= $rf[4]) {
            $rf[4]=$line[9]+$line[8];
          }
          if ($line[9]+$line[8]+($line[12]-$line[8]+1) >= $rf[5]) {
            $rf[5]=$line[9]+$line[8]+($line[12]-$line[8]+1);
          }
        } else {
          $rf[4]=$line[9]+$line[8];
          $rf[5]=$line[9]+$line[8]+($line[12]-$line[8]+1);
        ...
        $ff[6]++;
        if ($ff[1] >0 || $ff[2]>0){
          if ($line[7]+$line[6] <= $ff[1]) {
            $ff[1]=$line[7]+$line[6];
          }
          if ($line[7]+$line[6]+($line[10]-$line[6]+1) >= $ff[2]) {
            $ff[2]=$line[7]+$line[6]+($line[10]-$line[6]+1);
          }
        } else {
          $ff[1]=$line[7]+$line[6];
          $ff[2]=$line[7]+$line[6]+($line[10]-$line[6]+1);
       ...
       if ($ff[4] >0 || $ff[5]>0){
         if ($line[9]+$line[8] <= $ff[4]){
           $ff[4]=$line[9]+$line[8];
         }
         if ($line[9]+$line[8]+($line[12]-$line[8]+1) >= $ff[5]) {
           $ff[5]=$line[9]+$line[8]+($line[12]-$line[8]+1);
         }
       } else {
         $ff[4]=$line[9]+$line[8];
         $ff[5]=$line[9]+$line[8]+($line[12]-$line[8]+1);
